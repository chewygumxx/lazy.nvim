...
describe("runner", function()
        ...
        for i = 1, 10 do
            defs["test" .. i]  = {
                ...
                run = function(task)
                    table.insert(
                        runs,
                        { plugin = task.plugin.name, task = task.name }
                    )
                end,
            }
            defs["error" .. i] = {
    ...
    it("runs the pipeline", function()
        local runner = Runner.new({
            plugins = plugins,
            pipeline = { "test.test1", "test.test2" },
        })
        runner:start()
        runner:wait()
        assert.equal(4, #runs)
    ...
    it("waits", function()
        local runner = Runner.new({
            plugins = plugins,
            pipeline = { "test.test1", "wait", "test.test2" },
        })
        runner:start()
        runner:wait()
        assert.equal(4, #runs)
    ...
    it("handles async", function()
        local runner = Runner.new({
            plugins = plugins,
            pipeline = { "test.async1", "wait", "test.async2" },
        })
        runner:start()
        runner:wait()
        assert.equal(4, #runs)
    ...
    it("handles skips", function()
        local runner = Runner.new({
            plugins = plugins,
            pipeline = { "test.test1", "test.skip", "test.test2" },
        })
        runner:start()
        runner:wait()
    ...
    it("aborts on error", function()
        local runner = Runner.new({
            plugins = plugins,
            pipeline = { "test.test1", "test.error1", "test.test2" },
        })
        runner:start()
        runner:wait()
        assert.equal(4, #runs)

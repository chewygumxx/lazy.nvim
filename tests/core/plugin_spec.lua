...
local function resolve(plugin)
    ...
    local ret  = meta and type(meta.__index) == "table"
        and resolve(meta.__index)
        or {}
...
describe("plugin spec url/name", function()
    local tests = {
        ...
        {
            { "foo/bar", url = "123" },
            { [1] = "foo/bar", name = "bar", url = "123" },
        },
        {
            { url = "https://foobar" },
            { name = "foobar", url = "https://foobar" },
        },
        {
            {
                { url = "https://foo", name = "foobar" },
                { url = "https://foo" },
            },
...
describe("plugin spec opt", function()
    ...
    describe("deps", function()
        ...
        Config.options.defaults.lazy = false
        local dep_name_tests         = {
            {
                { "foo/bar", dependencies = { { "dep1" }, "foo/dep2" } },
                "foo/dep1",
            },
            {
                "foo/dep1",
        ...
        for _, test in ipairs(dep_name_tests) do
      ...
      -- },
        ...
        for _, test in ipairs(dep_order_tests) do
            it("handles dep names " .. inspect(test), function()
                local spec = Plugin.Spec.new(vim.deepcopy(test))
                assert(#spec.notifs == 0)
                Config.plugins = spec.plugins
        ...
        it("handles opt from dep", function()
            ...
            local spec                   = Plugin.Spec.new({
                ...
                {
                    "foo/bar",
                    dependencies = { "foo/dep1", "foo/dep2" },
                },
            })
            assert(#spec.notifs == 0)
            Config.plugins = spec.plugins
        ...
        it("merges lazy loaders", function()
            ...
            for _, test in ipairs(merge_tests) do
                ...
                local events = vim.tbl_keys(
                    spec.plugins.bar._.handlers.event or {}
                )
        ...
        local tests = {
            [{
                { "foo/bax" },
                {
                    "foo/bar",
                    optional = true,
                    dependencies = "foo/dep1",
                },
            }] = false,
            [{
                { "foo/bax", dependencies = "foo/dep1" },
                {
                    "foo/bar",
                    optional = true,
                    dependencies = "foo/dep1",
...
describe("plugin opts", function()
    -- config = {...} fixtures below intentionally exercise the deprecated
    -- table-config migration path, not the fun|true shape LazyPluginSpec expects.
...
describe("plugin spec", function()
    it("only includes fragments from enabled plugins", function()
        ...
        for _, test in ipairs(tests) do
            ...
            assert.same(
                test.expected_opts,
                Plugin.values(spec.plugins.bar, "opts")
            )
        end
    end)
end)

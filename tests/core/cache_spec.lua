...
describe("cache find", function()
  it("returns only the first match by default, all matches with opts.all", function()
    local paths = {
      Helpers.path("cache_all_a"),
      Helpers.path("cache_all_b"),
    }
    Helpers.fs_create({ "cache_all_a/lua/foo.lua", "cache_all_b/lua/foo.lua" })

    local first = Cache.find("foo", { rtp = false, paths = paths })
    assert.equal(1, #first)

    local all = Cache.find("foo", { rtp = false, all = true, paths = paths })
    assert.equal(2, #all)
  ...
  it("enumerates top-level modules for topmod == '*'", function()
    ...
    Helpers.fs_create({ "cache_star/lua/mod1.lua", "cache_star/lua/mod2.lua" })

    local all = Cache.find("*", { rtp = false, all = true, paths = { path } })
  ...
  it("prefers /init.lua over a flat .lua for single-segment modnames", function()
    ...
    local results = Cache.find("dirmod", { rtp = false, paths = { path } })
    assert.equal(1, #results)
    assert(results[1].modpath:find("init.lua", 1, true) ~= nil, results[1].modpath)
  ...
  it("prefers a flat .lua over /init.lua for dotted modnames", function()
    ...
    local results = Cache.find("pkg.dirmod", { rtp = false, paths = { path } })
    assert.equal(1, #results)
    assert(
      results[1].modpath:sub(-4) == ".lua" and not results[1].modpath:find("init.lua", 1, true),
      results[1].modpath
    )
  ...
  it("respects opts.patterns, skipping the default OPTIM patterns", function()
    local path = Helpers.path("cache_patterns")
    Helpers.fs_create({ "cache_patterns/lua/custommod/custom.lua" })

    local results = Cache.find("custommod", { rtp = false, patterns = { "/custom.lua" }, paths = { path } })
    assert.equal(1, #results)
  ...
  it("bumps the not_found stat when nothing matches", function()
    ...
    local before = Cache._inspect().find.not_found
    Cache.find("definitely_missing_xyz", { rtp = false, paths = { path } })
    local after = Cache._inspect().find.not_found
    assert.equal(before + 1, after)

    Helpers.fs_rm("cache_missing")
  end)

  it("M.reset(path) invalidates the stale top-level module index for that path", function()
    local path = Helpers.path("cache_reset_one")
    vim.fn.mkdir(path .. "/lua", "p")

    assert.equal(0, #Cache.find("newmod", { rtp = false, paths = { path } }))
    Helpers.fs_create({ "cache_reset_one/lua/newmod.lua" })
    assert.equal(0, #Cache.find("newmod", { rtp = false, paths = { path } }), "should still be stale")

    Cache.reset(path)
    assert.equal(1, #Cache.find("newmod", { rtp = false, paths = { path } }))
  ...
  it("M.reset() with no path invalidates every indexed path", function()
    ...
    assert.equal(0, #Cache.find("newmod", { rtp = false, paths = { path_a } }))
    assert.equal(0, #Cache.find("newmod", { rtp = false, paths = { path_b } }))
    Helpers.fs_create({ "cache_reset_all_a/lua/newmod.lua", "cache_reset_all_b/lua/newmod.lua" })

    Cache.reset()

    assert.equal(1, #Cache.find("newmod", { rtp = false, paths = { path_a } }))
    assert.equal(1, #Cache.find("newmod", { rtp = false, paths = { path_b } }))

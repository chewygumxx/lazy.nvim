...
describe("util", function()
    ...
    before_each(function()
        vim.opt.rtp = rtp
        for k, _v in pairs(package.loaded) do
        ...
        Helpers.fs_rm("")
        assert(
            not vim.uv.fs_stat(Helpers.path("")),
            "fs root should be deleted"
        )
    end)
    ...
    it("lsmod lists all mods in dir", function()
        ...
        for t, test in ipairs(tests) do
            ...
            table.sort(expected)
            Helpers.fs_rm("")
            local _files = Helpers.fs_create(test.files)
            ...
            assert(
                root,
                "no root found for " .. test.mod .. " (test " .. t .. ")"
            )
            assert.same(Helpers.path(test.root), root)
            local mods = {}
            Util.lsmod(test.mod, function(modname, _modpath)
                mods[#mods + 1] = modname
            end)
            table.sort(mods)
    ...
    it("find the correct root with dels", function()
        ...
        Helpers.fs_rm("old")
        assert(
            not vim.uv.fs_stat(Helpers.path("old/lua/foobar")),
            "old/lua/foobar should not exist"
        )

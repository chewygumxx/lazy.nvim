...
describe("fragments add", function()
    it("expands a slug via the patched url_format", function()
        MiniTest.finally(
            Mocks.patch_config({
                git = {
                    url_format = "https://example.test/%s.git",
                },
            })
        )
    ...
    it("migrates a deprecated table config to opts, with a warning", function()
        local spec, Fragments = new_spec()
        ---@diagnostic disable-next-line: assign-type-mismatch -- intentionally testing the deprecated table `config` migration path
    ...
    it(
        "marks dependency fragments as dep=true, spec children as not",
        function()
            local _, Fragments = new_spec()
    ...
    it(
        "keeps frag_stack/dep_stack balanced after nested deps-of-deps",
        function()
            local _, Fragments = new_spec()
            Fragments:add({
                "foo/top",

...
local function new_spec(opts)
    local spec = Plugin.Spec.new(
        nil,
        vim.tbl_extend("force", { pkg = false }, opts or {})
    )
...
describe("meta add", function()
    ...
    it(
        "handles a rename when a later fragment sets an explicit name",
        function()
            local spec, Meta = new_spec()
            Meta:add({ "foo/bar" })
...
describe("meta rebuild", function()
    ...
    it("resolves an explicit dir, ignoring dev", function()
        ...
        Meta:rebuild()
        assert.equal(
            Util.norm("/tmp/lazy-test-explicit-dir"),
            spec.plugins.bar.dir
        )
    end)
    ...
    it(
        "matches dev.patterns and uses dev.path when fallback is disabled",
        function()
            local restore = Mocks.patch_config({
                dev = {
                    patterns = { "devmatch" },
                    fallback = false,
                    path = "/tmp/lazy-test-dev",
                },
            })
            ...
            assert.equal(
                Util.norm("/tmp/lazy-test-dev/bar"),
                spec.plugins.bar.dir
            )
        end
    )
    ...
    it(
        "falls back to root when dev.fallback is enabled and the dev dir is missing",
        function()
            local restore = Mocks.patch_config({
                dev = {
                    patterns = { "devmatch" },
                    fallback = true,
                    path = "/tmp/lazy-test-dev-missing-xyz",
                },
            ...
            assert.equal(
                Util.norm("/tmp/lazy-test-root/bar"),
                spec.plugins.bar.dir
            )
        end
    )
...
describe("meta rebuild (dirty fragments)", function()
    ...
    it(
        "removes the plugin entirely when its only fragment is deleted",
        function()
            local spec, Meta = new_spec()
...
describe("meta fix_cond", function()
    it(
        "marks cond=false and cascades ignore_installed to dependencies",
        function()
            local spec, Meta = new_spec()
...
describe("meta fix_disabled", function()
    it(
        "disables a non-optional plugin, tracking it in spec.disabled",
        function()
            local spec, Meta = new_spec()
    ...
    it(
        "removes an optional disabled plugin without tracking it as disabled",
        function()
            local spec, Meta = new_spec()
...
describe("meta fix_optional", function()
    it(
        "short-circuits and returns 0 when the whole spec is optional",
        function()

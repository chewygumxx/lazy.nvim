...
function M.load(plugins, reason, opts)
    plugins = (type(plugins) == "string" or plugins.name) and { plugins }
        or plugins
...
function M.reload(plugin)
    ...
    local events = plugin._.handlers and plugin._.handlers.event
        and plugin._.handlers.event
        or {}

    for _, event in pairs(events) do
        if event.id:find("VimEnter") or event.id:find("UIEnter")
            or event.id:find("VeryLazy") then
            load = true
            break
        end
    end
    ...
    for _, s in ipairs(scripts) do
        ---@type string
        local path = s.name
        if path:sub(-4) == ".vim" and path:find(plugin.dir, 1, true) == 1
            and not path:find("/plugin/", 1, true)
            and not path:find("/ftplugin/", 1, true) then
...
function M._load(plugin, reason, opts)
    ...
    elseif reason.require then
        -- `loaded` is built ad hoc above from `reason`'s fields, so LuaLS infers
        -- its `source` field type from that loop instead of the formal class.
...
function M.config(plugin)
    ...
    if type(plugin.config) == "function" then
        ...
        if main then
            ...
            return Util.error(
                "Lua module not found for config of " .. plugin.name
                    .. ". Please use a `config()` function instead"
            )
        end
    end
...
function M.source_runtime(...)
    ...
    Util.walk(dir, function(path, name, t)
        ...
        name      = name:sub(1, -5)
        if (t == "file" or t == "link") and (ext == "lua" or ext == "vim")
            and not M.disabled_rtp_plugins[name] then
            files[#files + 1] = path
        end
    end)
...
function M.add_to_luapath(plugin)
    ...
    package.cpath = package.cpath .. ";" .. cpath .. "/?."
        .. (jit.os:find("Windows") and "dll" or "so") .. ";"
    package.cpath = package.cpath .. ";" .. cpath2 .. "/?."
        .. (jit.os:find("Windows") and "dll" or "so") .. ";"
end
    ...
    if plugin then
        ...
        if M.did_handlers and not (Plugin.loading or plugin._.loaded) then
            if plugin.module == false then
                error(
                    "Plugin " .. plugin.name
                        .. " is not loaded and is configured with module=false"
                )
            end
            M.load(plugin, { require = modname })
            if plugin._.cond == false then
                error(
                    "You're trying to load `" .. plugin.name
                        .. "` for which `cond==false`"
                )
            end
        end
    end

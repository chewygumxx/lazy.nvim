...
---@param cmd LazyCmd
function M:_add(cmd)
  vim.api.nvim_create_user_command(cmd.name, function(event)
    local command = {
      cmd = cmd.name,
    ...
    ---@type string
    local plugins = "`" .. table.concat(vim.tbl_values(self.active[cmd.id]), ", ") .. "`"

    self:_load(cmd)

    local info = vim.api.nvim_get_commands({})[cmd.name] or vim.api.nvim_buf_get_commands(0, {})[cmd.name]
    if not info then
      vim.schedule(function()
        Util.error("Command `" .. cmd.name .. "` not found after loading " .. plugins)
      end)
      return
    end

...
function M.setup(opts)
    ...
    if type(M.options.spec) == "string" then
        local spec_import = M.options.spec           --[[@as string]]
        M.options.spec    = { import = spec_import }
    end
    table.insert(M.options.install.colorscheme, "habamax")

    -- root
    M.options.root = Util.norm(M.options.root)
    local dev_path = M.options.dev.path
    if type(dev_path) == "string" then
        M.options.dev.path = Util.norm(dev_path)
    end
            ...
            callback = function()
                ...
                vim.api.nvim_create_autocmd("BufWritePost", {
                    ...
                    callback = function()
                        ...
                        local plugin = Plugin.find(vim.uv.cwd() .. "/lua/")
                        if plugin then

...
function Spec:import(spec)
    ...
    if type(spec.import) == "function" then
        if not spec.name then
            return self:error(
                "Invalid import spec. Missing name: " .. vim.inspect(spec)
            )
        end
    ...
    if spec.enabled == false
        or (type(spec.enabled) == "function" and not spec.enabled()) then
        return
    end
    ...
    if type(import) == "string" then
            ...
            modspecs[#modspecs + 1] = {
                ...
                load = function()
                    ...
                    if mod then
                        ...
                        if foo then
                            return nil,
                                "Spec module returned more than one value. Expected a single value."
                        end
                        return ret
                    else
                        return nil, err
        ...
        local import_load = spec.import                                  --[[@as fun():(LazyPluginSpec?, string?)]]
        modspecs          = {
            {
                modname = import_name,
                load = import_load,
    ...
    for _, modspec in ipairs(modspecs) do
        ...
        Util.try(function()
            ...
            elseif type(mod) ~= "table" then
                return self:error(
                    "Invalid spec module: `" .. modname
                        .. "`\nExpected a `table` of specs, but a `" .. type(
                            mod
                        )
...
function M.update_state()
    ...
    for _, plugin in pairs(Config.plugins) do
        ...
        if plugin.lazy == nil then
            local lazy  = plugin._.dep or Config.options.defaults.lazy
                or plugin.event or plugin.keys
                or plugin.ft or plugin.cmd
...
function M.update_rocks_state()
    local root = assert(Config.options.rocks.root)

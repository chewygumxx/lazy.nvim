...
function Loader.get_rtp()
    ...
    if key ~= Loader._rtp_key then
        ...
        for _, path in ipairs(vim.api.nvim_get_runtime_file("", true)) do
            ...
            if path:sub(-6, -1) ~= "/after"
                and not (Loader._indexed[path]
                    and vim.tbl_isempty(Loader._indexed[path])) then
    ...
    if f then
        ...
        return {
            hash = {
                ...
                mtime = {
                    sec = tonumber(header[3]),
                    nsec = tonumber(header[4]),
                },
            },
...
function Loader.loader_lib(modname)
    ...
    local is_win  = sysname:find("win", 1, true)
        and not sysname:find("darwin", 1, true)
    local ret     = M
        .find(modname, { patterns = is_win and { ".dll" } or { ".so" } })
        [1]
    ...
    if ret then
        ...
        local chunk, err = package.loadlib(
            ret.modpath,
            "luaopen_" .. funcname:gsub("%.", "_")
        )
...
function Loader.loadfile(filename, mode, env, hash)
    local start = uv.hrtime()
    filename    = filename and Loader.normalize(filename)
    mode        = nil                                     -- ignore mode, since we byte-compile the lua source files
    -- a nil filename (stdin, per stock `loadfile`) isn't actually implemented
    -- by Loader.load, this override only ever gets called with a real path
    ---@diagnostic disable-next-line: param-type-mismatch
    local chunk, err = Loader.load(filename, {
        mode = mode,
        env = env,
        hash = hash,
    })
...
function M.find(modname, opts)
    ...
    local patterns = opts.patterns
        or (topmod == modname and { "/init.lua", ".lua" }
            or { ".lua", "/init.lua" })
...
function M._profile_loaders()
    for l, loader in pairs(package.loaders) do
        local loc          = debug.getinfo(loader, "Sn")
            .source
            :sub(2)

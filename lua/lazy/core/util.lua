...
function M.pretty_trace(opts)
    ...
    while true do
        ...
        if info.what ~= "C"
            and (Config.options.debug or not info.source:find("lazy.nvim")) then
...
function M.get_source()
    ...
    while true do
        ...
        if info.what ~= "C" and not info.source:find("lazy.nvim", 1, true)
            and info.source ~= "@vim/loader.lua" then
...
function M.key_get(t, key)
    local key_str = key                                                                     --[[@as string]]
    local path    = type(key) == "table" and key
        or vim.split(key_str, ".", {
            plain = true,
        })
    local value   = t

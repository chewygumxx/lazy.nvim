    ...
    for _, pkg in ipairs(Pkg.get()) do
        ...
        if meta and fragment then
            ...
            for _, fid in ipairs(meta._.frags) do
                if fid > last_id then
                    local frag         = assert(self.fragments:get(fid))
...
function M:rebuild()
    ...
    if Config.options.debug then
        Util.track(
            "rebuild plugins frags=" .. frag_count
...
function M:_rebuild(name)
    ...
    for _, fid in ipairs(plugin._.frags) do
        if not added[fid] then
            ...
            assert(
                fragment,
                "fragment " .. fid .. " not found, for plugin " .. name
            )
            super           = setmetatable(
                fragment.spec,
                super and { __index = super } or nil
            )
            plugin._.dep    = plugin._.dep and fragment.dep
            plugin.optional = plugin.optional
                and (rawget(fragment.spec, "optional") == true)
    ...
    elseif super.virtual then
        ...
        if plugin.dev == true then
            local dev_dir = type(Config.options.dev.path) == "string"
                and Config.options.dev.path .. "/" .. plugin.name
                or Util.norm(Config.options.dev.path(plugin))
            if not Config.options.dev.fallback
                or vim.fn.isdirectory(dev_dir) == 1 then
                plugin.dir = dev_dir
            else
                plugin.dev = false
            end
    ...
    local function check(top)
        for _, plugin in pairs(self.plugins) do
            if (plugin._.top or false) == top then
                ...
                if enabled == false
                    or (type(enabled) == "function" and not enabled(plugin)) then
                    changes = changes + 1
                    if plugin.optional then
                        self:del(plugin.name)
                    else

...
function M.schedule()
    ...
    local next_check = State.checker.last_check
        + Config.options.checker.frequency
        - os.time()
...
function M.report(notify)
    ...
    if #lines > 0 and notify and Config.options.checker.notify
        and not Config.headless() then

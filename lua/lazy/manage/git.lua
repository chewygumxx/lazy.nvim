...
function M.get_versions(repo, spec)
    local range = assert(Semver.range(type(spec) == "string" and spec or "*"))
    ---@type TaggedSemver[]
    local versions = {}
...
function M.get_commit(repo, branch, origin)
    if origin then
        ...
        return M.ref(repo, "remotes/origin", branch)
            or M.ref(repo, "heads", branch)
    else
...
function M.get_target(plugin)
    ...
    local version = (plugin.version == nil and plugin.branch == nil)

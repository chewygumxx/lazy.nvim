...
function Task:headless()
    ...
    io.write(
        Terminal.prefix(color and color(msg.msg) or msg.msg, self:prefix())
    )
    io.write("\n")
end
...
function Task:_done()
    if Config.headless() and Config.options.headless.task then
        ...
        self:log(
            "Finished task " .. self.name .. " in " .. ms .. "ms",
            vim.log.levels.INFO
        )
    end
    ...
    vim.schedule(function()
        vim.api.nvim_exec_autocmds("User", {
            pattern = "LazyPlugin" .. self.name:sub(1, 1):upper()
                .. self.name:sub(2),
...
function Task:spawn(cmd, opts)
    ...
    if not headless then
        ...
        if #msg > 0 then
            self:log(
                vim.trim(proc.data),
                ok and vim.log.levels.DEBUG or vim.log.levels.ERROR
            )
        end
    end
...
function Task:prefix()
    ...
    local task   = string.rep(" ", 20 - #(self.name .. self.plugin.name))
        .. self.name

    return colors
        and Terminal.magenta(plugin) .. Terminal.cyan(task)

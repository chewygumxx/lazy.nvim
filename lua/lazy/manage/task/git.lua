...
function throttle.next()
    throttle.running = 0
    while #throttle.waiting > 0
        and throttle.running < Config.options.git.throttle.rate do
        ---@type Async
...
function throttle.wait()
    ...
    if not throttle.timer:is_active() then
        throttle.timer:start(
            0,
            Config.options.git.throttle.duration,
            vim.schedule_wrap(throttle.next)
        )
    end
    local running = assert(Async.running())
...
M.log = {
    ...
    skip = function(plugin, opts)
        ...
        if opts.updated
            and not (plugin._.updated
                and plugin._.updated.from ~= plugin._.updated.to) then
            return true
        end
    ...
    run = function(self, opts)
        ...
        if opts.updated then
            table.insert(
                args,
                self.plugin._.updated.from .. ".."
                    .. (self.plugin._.updated.to or "HEAD")
            )
        elseif opts.check then
            ...
            if not self.plugin._.is_local then
                if Git.eq(info, target) then
                    if Config.options.checker.check_pinned then
                        local last_commit = Git.get_commit(
                            self.plugin.dir,
                            target.branch,
                            true
                        )
            ...
            vim.list_extend(args, opts.args or assert(Config.options.git.log))
        end
                ...
                if ok then
                    ...
                    lines = vim.tbl_filter(function(line)
                        ...
                        if line:gsub("[\\/]", "/") == "doc/tags" then
                            ...
                            local checkout_opts = { cwd = self.plugin.dir }
                            Process.exec(
            ...
            on_exit = function(ok)
                if ok then
                    ...
                    if not self.plugin._.cloned then
                        ...
                        if self.plugin._.updated.from
                            ~= self.plugin._.updated.to then
                            self.plugin._.dirty = true
                        end
                    end
                end

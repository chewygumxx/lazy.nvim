...
function Process.new(cmd, opts)
    ...
    opts.timeout = opts.timeout
        or (Config.options.git and Config.options.git.timeout * 1000)
...
function Process:_run()
    ...
    if self.handle then
        ...
        self:suspend()
        while not (self.handle:is_closing() and stdout:is_closing()
            and stderr:is_closing()) do
            Async.yield()
        end
    else
        self.data = "Failed to spawn process " .. self.cmd
...
function Process:on_exit()
    ...
    elseif self.signal ~= 0 then
        self.data = self.data .. "\n" .. "Process was killed with SIG"
            .. M.signals[self.signal]:upper()
    end
...
function Process:on_data(err, data, is_stderr)
    ...
    local lines = vim.split(
        vim.trim(self.data:gsub("\r$", "")):gsub("[^\n\r]+\r", ""),
        "\n"
    )

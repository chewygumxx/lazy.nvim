...
function M.weak(obj)
    ...
    local function get()
        ...
        return ret == nil and error("Object has been garbage collected", 2)
            or ret
    end
    local mt = {
        __mode = "v",
...
function M.float_term(cmd, opts)
    ...
    -- vim.fn.jobstart's term=true replacement needs nvim 0.11+; termopen still
    -- works everywhere this plugin supports, so keep it rather than version-branch.
...
function M.open_cmd()
    M.warn(
        [[`require("lazy.util").open_cmd()` is deprecated. Please use `float_term` instead. Check the docs]]
    )
end

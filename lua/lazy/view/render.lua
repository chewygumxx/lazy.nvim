...
function M.new(view)
    local self   = setmetatable({}, {
        __index = setmetatable(M, { __index = Text }),
    }) --[[@as LazyRender]]
    self.view    = view
    self.padding = 2
...
function M:title()
    ...
    if Config.options.ui.pills then
        ...
        for c, mode in ipairs(modes) do
            local title = " " .. mode.name:sub(1, 1):upper()
                .. mode.name:sub(2) .. " ("
                .. mode.key .. ") "
    ...
    if self.view.state.mode ~= "help" and self.view.state.mode ~= "profile"
        and self.view.state.mode ~= "debug" then
        if self.progress.done < self.progress.total then
...
function M:help()
    self:append("Help", "LazyH2")
        :nl()
        :nl()

    self:append("Use ")
        :append(ViewConfig.keys.abort, "LazySpecial")
        :append(" to abort all running tasks.")
        :nl()
        :nl()

    self:append("You can press ")
        :append("<CR>", "LazySpecial")
    ...
    self:append("When hovering with ")
        :append("<K>", "LazySpecial")
        :append(" on a plugin anywhere else, a diff will be opened if there are updates")
        :nl()
    self:append("or the plugin was just updated. Otherwise the plugin webpage will open.")
        :nl()
        :nl()

    self:append("Use ")
        :append("<d>", "LazySpecial")
    ...
    self:nl()
        :append("Keyboard Shortcuts for Plugins", "LazyH2")
        :nl()
    for _, mode in ipairs(ViewConfig.get_commands()) do
...
function M:progressbar()
    ...
    local done  = math.floor(
        (self.progress.done / self.progress.total) * width + 0.5
    )
    if self.progress.done == self.progress.total then
        done = 0
    end
...
function M:section(section)
    ...
    if count > 0 then
        self:append(section.title, "LazyH2")
            :append(" (" .. count .. ")", "LazyComment")
            :nl()
...
function M:reason(reason, opts)
        ...
        if plugin then
            ...
            if source == config .. "/init.lua" then
                ...
                if source:find(config, 1, true) == 1 then
                    reason.source = source:sub(#config + 2)
                        :gsub("/", ".")
                        :gsub("%.lua$", "")
                end
            end
    ...
    local time = reason.time
        and (" "
            .. self:ms(
                reason.time --[[@as number]]
            ))
    ...
    for _, task in ipairs(plugin._.tasks or {}) do
        if task:running() then
            self:diagnostic({
                ...
                message = task.name
                    .. (task:status() and (": " .. task:status()) or ""),
            })
            skip = true
    ...
    elseif plugin._.updated then
        if plugin._.updated.from == plugin._.updated.to then
            ...
            if version then
                ...
                self:diagnostic({
                    message = "updated from " .. plugin._.updated.from:sub(1, 7)
                        .. " to " .. plugin._.updated.to:sub(1, 7),
                })
            end
        end
...
function M:plugin(plugin)
    ...
    if plugin._.loaded then
        self
            :append("  " .. Config.options.ui.icons.loaded .. " ", hl)
            :append(plugin.name)
    elseif plugin._.cond == false then
        self
            :append("  " .. Config.options.ui.icons.not_loaded .. " ", "LazyNoCond")
            :append(plugin.name)
    else
        self
            :append("  " .. Config.options.ui.icons.not_loaded .. " ", hl)
            :append(plugin.name)
    end
...
function M:tasks(plugin)
    for _, task in ipairs(plugin._.tasks or {}) do
        if self.view:is_selected(plugin) then
            self:append(Config.options.ui.icons.task .. "[task] ", "Title", {
                indent = 4,
            }):append(task.name)
            self:append(
                " " .. math.floor((task:time()) * 100) / 100 .. "ms",
                "Bold"
            )
            self:nl()
        end
...
function M:details(plugin)
    ...
    if plugin.url then
        table.insert(
            props,
            { "url", (plugin.url:gsub("%.git$", "")), "LazyUrl" }
        )
    end
    ...
    if git then
        ...
        if git.commit then
            table.insert(
                props,
                { "commit", git.commit:sub(1, 7), "LazyCommit" }
            )
        end
    end
...
function M:props(props, opts)
    ...
    local prop_opts = { indent = opts.indent or 0 }
    for _, prop in ipairs(props) do
        self:append(
            prop[1] .. string.rep(" ", width - #prop[1] + 1),
            "LazyProp",
            prop_opts
        )
...
function M:profile()
    ...
    self:append("Startuptime: ", "LazyH2")
        :append(ms .. "ms", "Number")
        :nl()
        :nl()
    if stats.real_cputime then
        self
            :append("Based on the actual CPU time of the Neovim process till ")
        ...
        self
            :append("An accurate startuptime based on the actual CPU time of the Neovim process is not available.")
            :nl()
        self
    ...
    for p, prop in ipairs(times) do
        if p > 1 then
            prop[2] = prop[2] .. " (+"
                .. self:ms((prop.time - times[p - 1].time) * 1e6) .. ")"
        end
    end
    ...
    local function _profile(entry, depth)
        ...
        self:append(("  "):rep(depth))
            :append(symbol, "LazySpecial")
            :append(" ")
...
function M.list_icon(depth)
    local symbols = assert(Config.options.ui.icons.list)
...
function M:debug()
    ...
    local handlers = require("lazy.core.handler").handlers
    Util.foreach(handlers, function(handler_type, handler)
        Util.foreach(handler.active, function(value, plugins)
            if not vim.tbl_isempty(plugins) then
                ...
                table.sort(plugins)
                self:append(
                    Config.options.ui.icons.debug,
                    "LazySpecial",
                    { indent = 2 }
                )
                if handler_type == "keys" then
                    for k, v in pairs(
                        Config.plugins[plugins[1]]._.handlers.keys
                    ) do
                        if k == value then
    ...
    Util.foreach(require("lazy.core.cache")._inspect(), function(name, stats)
        ...
        local props = {
            ...
            {
                "avg time",
                self:ms((stats.time or 0) / (stats.total or 0), 3),
                "Bold",
            },
        }

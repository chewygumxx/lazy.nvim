...
local M_table = M --[[@as table]]
setmetatable(M_table, {
    __call = function(_, ...)
        return M.new(...)
    end,
})
...
function M:layout()
    ...
    if self.opts.margin then
        ...
        if self.opts.margin.bottom then
            self.win_opts.height = self.win_opts.height
                - self.opts.margin.bottom
        end
...
function M:mount()
    ...
    if vim.fn.has("nvim-0.9.0") == 0 then
        ---@diagnostic disable-next-line: deprecated -- pre-0.9 fallback, intentional
    ...
    if has_bg and self.opts.backdrop
        and self.opts.backdrop < 100 and vim.o.termguicolors then
    ...
    self.win = vim.api.nvim_open_win(
        self.buf,
        true,
        self.win_opts --[[@as vim.api.keyset.win_config]]
    )
    ...
    vim.api.nvim_create_autocmd(
        "VimResized",
        {
            callback = function()
                ...
                if self.backdrop_win
                    and vim.api.nvim_win_is_valid(self.backdrop_win) then
...
function M:augroup(clear)
    return vim.api.nvim_create_augroup("trouble.window." .. self.id, {
        clear = clear == true,
    })
end

...
function M.setup(spec, opts)
    ...
    local notify_opts = { title = "lazy.nvim" }
    ...
    if not (pcall(require, "ffi") and jit and jit.version) then
        return vim.notify(
            "lazy.nvim requires Neovim built with LuaJIT",
            vim.log.levels.ERROR,
            notify_opts
        )
    end
    ...
    local enable_cache = vim.tbl_get(opts, "performance", "cache", "enabled")
        ~= false
    ...
    if Config.plugins["lazy.nvim"] then
        Config.plugins["lazy.nvim"]._.loaded = {
            time = delta,
            source = "init.lua",
        }
    end
    ...
    vim.api.nvim_exec_autocmds(
        "User",
        { pattern = "LazyDone", modeline = false }
    )
...
setmetatable(M, {
    __index = function(_, key)
        return function(...)
            return require("lazy.view.commands")
                .commands
                [key](...)
        end
    end,

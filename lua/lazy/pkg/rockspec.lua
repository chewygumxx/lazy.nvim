...
function M.build(task)
    ...
    assert(
        pkg,
        "missing rockspec pkg for " .. task.plugin.name
...
function M.is_simple_build(rockspec)
    ...
    return type == nil or type == "none"
        or (type == "builtin" and not rockspec.build.modules)
end
...
function M.get(plugin)
    ...
    if not rockspec or not rockspec_file then
        return
    end

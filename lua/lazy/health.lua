...
function M.have(cmd, opts)
        ...
        if vim.fn.executable(c) == 1 then
            ...
            if exit_code ~= 0 then
                ...
                if opts.version_pattern
                    and not version:find(opts.version_pattern, 1, true) then
                    opts.warn(
                        ("`%s` version `%s` needed, but found `%s`"):format(
                            c,
    ...
    if found then
        ...
        local version_note = ""
        if opts.version_pattern then
            version_note = "version `" .. opts.version_pattern .. "` "
        end
...
function M.check()
    ...
    for _, name in ipairs({ "packer", "plugged", "paq", "pckr", "mini.deps" }) do
        for _, path in ipairs(
            ...
            if path:find(name, 1, true) then
                error(
                    "Found paths on the rtp from another plugin manager `"
                        .. name
                        .. "`"
                )
                break
    ...
    local packer_compiled = vim.fn.stdpath("config")
    ...
    if spec == nil then
        error(
            'No plugins loaded. Did you forget to run `require("lazy").setup()`?'
        )
    else
    ...
    if Config.options.rocks.enabled then
        ...
        if #need_luarocks == 0 then
            ok(
                "no plugins require `luarocks`, so you can ignore any warnings below"
            )
        else
        ...
        local rockspec_ok = require("lazy.pkg.rockspec").check({
            error = #need_luarocks > 0 and error or warn,
            warn = warn,
            ok = ok,
        })
        if not rockspec_ok then
            warn(
                table.concat({
                    ...
                    Config.hererocks()
                        and " - disable *hererocks* with `opts.rocks.hererocks = false`"
